...
Windows-приложение для тренировки интервью и учебных демонстраций. Оно распознаёт речь собеседника и пользователя, выделяет вопросы и показывает потоковый ответ модели в компактной панели. Используйте его с согласия участников и в рамках правил занятия или интервью.

**[Скачать Setup.exe — Releases](https://github.com/BLNCname/interview-assistant/releases/latest)** · [Как работает сетевой установщик](docs/ONLINE_INSTALLER_RU.md) · [Требования к GPU и VRAM](docs/HARDWARE_RU.md) · [Запуск и сборка из исходников](docs/BUILD_RU.md)

Распознавание работает локально на модели `large-v3-turbo`, которую загружает установщик. Для ответов можно выбрать **OpenRouter** или **LM Studio**. Поиск через официальные **Firecrawl MCP** и **Context7 MCP** встроен в приложение: отдельные MCP-плагины и Node.js не нужны. Все четыре ключа вводятся в интерфейсе; `.env` для обычной установки не требуется.
...
| Диск | Рекомендуется 12 ГБ свободного места для установки, кеша и временных файлов. Модели LM Studio требуют дополнительного места |
| Интернет при установке | Около 3,1 ГБ загрузок с PyPI и Hugging Face; время зависит от соединения |
...
`Setup.exe` содержит ядро приложения и базовый Python; во время установки загружает закреплённые версии библиотек, включая Qt и CUDA/cuDNN, и STT-модель. Устанавливать Git, Python, Node.js или CUDA Toolkit вручную не нужно. Драйвер NVIDIA, LM Studio и веса модели для генерации ответов устанавливаются отдельно. Для облачного варианта LM Studio не требуется.
...
1. Откройте [последний релиз](https://github.com/BLNCname/interview-assistant/releases/latest). Репозиторий приватный: войдите в браузере под аккаунтом с доступом к нему.
2. Для версии **0.1.4** в **Assets** скачайте один файл **`Setup.exe`**. Архивы `Source code` для установки не нужны.
3. Сверьте SHA-256 с контрольной суммой в описании этого релиза. В PowerShell из папки загрузки выполните:

   ```powershell
   Get-FileHash -LiteralPath .\Setup.exe -Algorithm SHA256
   ```

4. Запустите `Setup.exe` и пройдите мастер. На странице **Speech recognition hardware** будут показаны видеокарта, память, драйвер и рекомендованный режим STT. Дождитесь загрузки зависимостей и модели: суммарно около **3,1 ГБ**. Сам мастер не запрашивает GitHub-токен или вход в GitHub. По умолчанию приложение устанавливается для текущего пользователя в `%LOCALAPPDATA%\Programs\InterviewAssistant`.
5. Запустите **Interview Assistant** через меню «Пуск» и настройте его по следующим разделам.

Если загрузка прервалась, запустите тот же `Setup.exe` снова. Полностью скачанные файлы сохраняются в `%LOCALAPPDATA%\InterviewAssistant\InstallerCache` и используются после проверки SHA-256. Незавершённый файл скачивается с начала. Размеры и устройство кеша описаны в [инструкции к сетевому установщику](docs/ONLINE_INSTALLER_RU.md).
...
Панель пропускает клики в обычном режиме. Для перемещения, изменения размера, выделения текста и прокрутки включите редактирование через `Ctrl+Shift+I`. Скрытие панели не останавливает STT: для этого используйте паузу. Для выхода включите взаимодействие через `Ctrl+Shift+I` и нажмите **Quit** на панели. Если панель скрыта, сначала покажите её через `Ctrl+Shift+O`.
...
| OpenRouter не принимает ключ | Сначала выберите OpenRouter, затем вставьте ключ и нажмите Run checks. [Разбор сообщений диагностики](docs/OPENROUTER_TROUBLESHOOTING_RU.md) |
...
| Загрузка установщика прервалась | Проверьте соединение и место на диске, затем запустите тот же Setup.exe: целые проверенные файлы сохраняются в кеше |
| Установщик сообщает несовпадение хеша | Повторите загрузку; не обходите проверку. При повторной ошибке сохраните текст ошибки и сообщите автору релиза |
...
В [нагрузочных испытаниях](docs/validation/stress-and-release-2026-09-08.md) конкретной конфигурации LM Studio сложные вопросы дали первый текст за 7,3–13,6 с и полный ответ за 10,3–18,2 с. Серия поиска списала 12 кредитов Firecrawl. В [живом тесте OpenRouter](docs/validation/release-0.1.4.md#живой-openrouter) бесплатная `cohere/north-mini-code:free` прошла четыре проверки провайдера и ответила на русский вопрос: первый текст за 2,42 с, полный ответ за 3,26 с, стоимость $0. Это результаты отдельных прогонов, а не гарантии скорости, доступности или расхода.

Другие отчёты: [STT и записи](docs/validation/recording-replay-2026-09-07.md), [качество ответов](docs/validation/heavy-answer-quality-2026-09-08.md), [предыдущий релиз 0.1.2](docs/validation/release-0.1.2.md). Доказательства для отдельных выпусков находятся в [каталоге испытаний](docs/validation/).
...
Кеш установщика сохраняется отдельно. После закрытия `Setup.exe` его можно удалить из `%LOCALAPPDATA%\InterviewAssistant\InstallerCache`, если повторная установка не планируется; при следующей установке файлы придётся загрузить заново.
